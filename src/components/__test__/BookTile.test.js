...
test("BookTile", () => {
  let book = Object.assign({}, Book);
  const {
    volumeInfo: { title, subtitle, imageLinks, authors }
  } = book;
  ...
  expect(wrapper.find(".book-tile-img").html()).toContain(
    "http://books.google.com/books/content?id=aGjaBTbT0o0C&amp;printsec=frontcover&amp;img=1&amp;zoom=1&amp;edge=curl&amp;source=gbs_api"
  );
  expect(
    wrapper.find(".book-tile-img").filterWhere(item => {
      return item.prop("src") === imageLinks.thumbnail;
    })
  ).toHaveLength(1);
});
// TODO description not picked as popover rendered outside wrapper
test("BookTile", () => {
